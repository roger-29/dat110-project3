...
public class TestConcurrentWriteWriteConsistencyPass {
	...
	public void test() throws RemoteException, InterruptedException {
		...
		// GOAL: A write/write operation must not leave the distributed data store in an
		// inconsistent state
		// We must not have a read-write or a write-write conflict under concurrent
		// operations
		...
		assertFalse(result); // This must assert to false (pass) i.e. pr1 and pr2 = false (at least one must
								// fail to avoid write/write conflicts)
	}
	...
	class client1 extends Thread {
		...
		public void run() {
			try {
				...
				boolean decision = recprocess.requestWriteOperation(wmessage); // send request and get back the voters
																				// decision // send a write request from
																				// p4
	...
	class client2 extends Thread {
		...
		public void run() {
			try {
				...
				boolean decision = recprocess.requestReadOperation(wmessage); // send request and get back the voters
																				// decision
				wmessage.setAcknowledged(decision); // put the voters' decision in a message
				recprocess.multicastVotersDecision(wmessage); // multicast the decision to others
				succeeded = performWriteOperation(recprocess, wmessage); // perform a write operation
