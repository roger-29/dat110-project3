package no.roger.dat110.clients;
...
import no.roger.dat110.mutexprocess.Config;
import no.roger.dat110.mutexprocess.Message;
import no.roger.dat110.mutexprocess.OperationType;
import no.roger.dat110.mutexprocess.Operations;
import no.roger.dat110.mutexprocess.ProcessContainer;

public class TestWriteReadConsistencyPass {
	...
	public void test() throws RemoteException {
		...
		// GOAL: A write operation must not leave the data store in an inconsistent
		// state
		// We must not have a read-write or a write-write conflict

		// we will manually manipulate the processes and acquire locks so that such
		// processes can deny requests
		// for N = 10 processes, we need 6 processes to grant permission (N/2 + 1) for
		// any READ or WRITE operation
		// NR + NW > N: infact (NR + NW) = N + 2 . We should not have read-write or
		...
		assertTrue(succeed); // must pass

		// GOAL: A read operation must produce exactly the same copy of data from any of
		// the replicas after a write operation
		...
		// most important is that the previous write operation must leave the data in a
		// consistent state - all versions must be the same
		assertEquals(p1.getVersion(), p2.getVersion());
		assertEquals(p1.getVersion(), p3.getVersion());
		assertEquals(p1.getVersion(), p4.getVersion());
		assertEquals(p1.getVersion(), p5.getVersion());
		assertEquals(p1.getVersion(), p6.getVersion());
		assertEquals(p1.getVersion(), p7.getVersion());
		assertEquals(p1.getVersion(), p8.getVersion());
		assertEquals(p1.getVersion(), p9.getVersion());
		assertEquals(p1.getVersion(), p10.getVersion());
	}
	...
	private boolean performReadOperation(ProcessInterface p, Message message) throws RemoteException {
		...
		if (message.isAcknowledged()) { // majority acknowledged?
			...
			Operations op = new Operations(p, message); // perform operation
			op.performOperation(); // on local resource
			p.multicastUpdateOrReadReleaseLockOperation(message); // multicast release lock to replica processes that
																	// voted
			p.releaseLocks(); // release locks to CS after operations
