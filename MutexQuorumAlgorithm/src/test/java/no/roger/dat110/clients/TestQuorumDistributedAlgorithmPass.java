...
public class TestQuorumDistributedAlgorithmPass {
	...
	public void test() throws Exception {
		...
		// No operation (READ/WRITE) should be allowed with this setup (i.e. 5 processes
		// are busy)
		p1.acquireLock();
		p2.acquireLock();
		p3.acquireLock();
		...
		assertFalse(decision); // this must assert to false (i.e pass)
		...
		p4.releaseLocks();
		p5.releaseLocks();
		// release locks from other processes not released because we did not perform
		// any real operation
		p6.releaseLocks();
		p7.releaseLocks();
		p8.releaseLocks();
		...
		assertTrue(decision); // this must assert to true (i.e pass)
	}
